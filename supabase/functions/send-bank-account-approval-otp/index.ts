...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!parentUser.phone) {
      return new Response(
        JSON.stringify({ error: 'Parent phone number not found. Please update parent profile with a phone number.' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }

    // Normalize phone number (ensure E.164 format with +)
    const normalizedPhone = parentUser.phone.trim().replace(/\s+/g, '')
    const parentPhone = normalizedPhone.startsWith('+') ? normalizedPhone : `+${normalizedPhone}`
    ...
    formData.append('To', parentPhone)
    ...
    return new Response(
      JSON.stringify({ 
        ...
        expires_at: expiresAt.toISOString(),
        parent_phone_masked: parentPhone.replace(/(\+\d{1,3})(\d{3})(\d{3})(\d{4})/, '$1***$2****') // Mask phone for privacy
      }),
      { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  } catch (error: any) {
