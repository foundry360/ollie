...
serve(async (req) => {
  ...
  try {
    ...
    const { conversation_sid, service_sid } = await req.json();

    if (!conversation_sid || !service_sid) {
      return new Response(
    ...
    // Fetch messages from Twilio Conversations REST API
    ...
    const messages = (data.messages || []).map((msg: any) => ({
      sid: msg.sid,
      index: parseInt(msg.index) || 0,
      author: msg.author || '',
      body: msg.body || '',
      dateCreated: new Date(msg.date_created),
      dateUpdated: new Date(msg.date_updated),
