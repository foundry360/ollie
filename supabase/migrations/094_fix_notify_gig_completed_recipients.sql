-- Migration: Fix notify_gig_completed to ensure correct recipient IDs
-- This ensures neighbors only get neighbor notifications and teens only get teen notifications

-- Drop and recreate the function to ensure it's using the correct logic
...
BEGIN
  IF NEW.status = 'completed' AND OLD.status != 'completed' AND NEW.teen_id IS NOT NULL THEN
    ...
    -- CRITICAL: Store recipient IDs in explicit variables BEFORE any function calls
