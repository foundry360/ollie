-- Fix: Update trigger to support OneSignal and ensure it's on the correct table
-- Also add additional deduplication at the trigger level

-- First, ensure the trigger is on the correct table (gigs)
DROP TRIGGER IF EXISTS on_new_gig_created_notify ON public.gigs;

-- Update the trigger function to:
-- 1. Check for both expo_push_token AND onesignal_user_id
-- 2. Add additional deduplication check before calling send_push_notification
...
BEGIN
  IF TG_OP = 'INSERT' AND NEW.status = 'open' THEN
    ...
    -- Notify all teenlancers who have push tokens (Expo OR OneSignal)
    -- CRITICAL: Track which users we've notified to prevent duplicates in the same execution
    FOR v_teenlancer IN
      SELECT DISTINCT 
        u.id, 
        u.expo_push_token,
...
-- Create trigger on the correct table (gigs)
CREATE TRIGGER on_new_gig_created_notify
  AFTER INSERT ON public.gigs
  FOR EACH ROW
  WHEN (NEW.status = 'open')
  EXECUTE FUNCTION notify_new_gig_created();
